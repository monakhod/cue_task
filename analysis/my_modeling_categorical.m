...
regTab.cueCategory = reordercats(regTab.cueCategory, {'neutral', 'valid', 'invalid'});
%% regress by cue
% vars
...
formulas2 = '%s~ 1+ cueCategory*initAcc  + (1 | pp)';
...
%% cue*certainty
varNames = { 'initAccLogistic','RT', 'cpp',}; nVars = length(varNames);
f = '%s ~ 1 +  cueCategory*certainty + (1 | pp) + (-1 + certainty| pp)';

glmeOpts = {'Distribution','binomial','Link','logit'};


    for iV = 1:nVars
        formula = sprintf(f, varNames{iV});
        fits3{iV} = fitglme(regTab, formula);
    end
...
varNames = {'initAccLogistic', 'cpp'}; nVars = length(varNames); % just these'RT'
formulas2 = '%s~ 1+ cueCategory + certainty + RT + (1 | pp) + (-1 + certainty| pp)';%
...
%% do regressions
varNames = { 'initAccLogistic','certainty', 'cpp',}; nVars = length(varNames);
f = '%s ~ 1 +  cueCategory*RT + (1 | pp)+ (-1 + RT | pp)';
...
formulas2 = '%s~ 1+ initAcc*RT + initAcc*cueCategory + RT*cueCategory + initAcc:RT:cueCategory + (1  +RT  +initAcc | pp)';
...
formulas2 = '%s~ 1+ cueCategory*certainty*initAcc  + (1 | pp)+ (-1 + certainty| pp)';
