...
regTab1.cueCategory = reordercats(regTab1.cueCategory, {'neutral', 'valid', 'invalid'});
%% only cue
...
formulas6 = '%s ~ 1 + cueCategory*RT + (1 | pp)+ (-1 + RT | pp)';
...
formulas2 = '%s~ 1+ cueCategory*certainty + (1 | pp)+ (-1 + certainty | pp)';%
...
formulas7 = '%s~ 1+ cueCategory*certainty*RT + (1 | pp)+ (-1 + certainty | pp) + (-1 + RT | pp)';%
...
regTables4.p = array2table(sq(nancat(cellfun(@(x) x.Coefficients.pValue(2:end), fits2, 'UniformOutput',0))),'VariableNames',varNames,'RowNames',fits2{1}.CoefficientNames(2:end)); 
regTables4.beta = array2table(sq(nancat(cellfun(@(x) x.Coefficients.Estimate(2:end), fits2, 'UniformOutput',0))),'VariableNames',varNames,'RowNames',fits2{1}.CoefficientNames(2:end)); 
