...
baseFolder = 'G:\cue_task\analysis\Data';
folder1 = fullfile(baseFolder, wh, 'beh');


f = what(folder1);
% files = f.mat(cellRegexpi(f.mat, '15.23')>0);
files = f.mat(cellfun(@(x) ~isempty(regexpi(x, wh, 'once')) && contains(x, '2024'), f.mat));
...
for i = 1:length (a)
    b = a{i};
    %if ~strcmp(b, 'pp')  % Skip the 'block' field
        behData.(b)(isGood_g == 0) = NaN;
   % end
end
...
% accs
ax = nexttile(t);
h = errorBarPlot(behData.initAcc, 'area',1);
xlabel('trial');
ylabel('accuracy');
...
ax = nexttile(t);
h = errorBarPlot(behData.RT, 'area',1);
xlabel('trial');
ylabel('RT (s)');
...
ax = nexttile(t);
errorBarPlot(behData.certainty, 'area',1);
xlabel('trial');
ylabel('certainty');
...
% 
ax = nexttile(t);
errorBarPlot(behData.score, 'area',1);
xlabel('trial');
ylabel('score');
...
%% making matrix with different conditions + corr for trails next to each other
 c = 1;
 all_con = [];
all_con = NaN(size (behData.block));
...
[mnt, cndtn] = CountUnique(many_cond); 

%deleting everything that less then 10
n = find (mnt<10)
if ~isempty (n)
    for i = 1 :length (n)
        many_cond(many_cond==cndtn(n(i))) = NaN;
    end
end

many_cond (many_cond==0)= NaN; 
[mnt1, cndtn1] = CountUnique(many_cond);
a = [mnt1, cndtn1]

mnt_cell = num2cell(mnt(1:end));
combined_data = [combined_labels, mnt_cell];
t_for_tea = array2table(combined_data, 'VariableNames', {'type_of_answer', 'amount'})
...
%matrix for chosen fr
freq_ch = behData.chosenFreq';
freq_ch = reshape (freq_ch, [], 1);

%matrix for chosen fr
freq_correct = behData.corrFreq';
freq_correct = reshape (freq_correct, [], 1);


%matrix for left or right answer
lr_resp = behData.respLR';
lr_resp = reshape (lr_resp, [], 1);
...
cue_and_cert (cue_and_cert==0)= NaN;

p = wh;
pp = repmat({p}, length (cue_and_cert), 1);
pp1 = cellfun(@(x) str2num(x(2:3)), pp);

%% make table

behTab = array2table ([pp1, e_or_c,cert, RT_p, cue_m], 'VariableNames',{ 'pp', 'initAcc', 'certainty', 'RT', 'cue'});
 behVarNames = {'initAcc','RT','certainty'};%'confAcc','confRT','confInR1',,'CoM','confInCorr','conf3'
% nBehVars = length(behVarNames);
% behDataNames = fieldnames(behData); % names of all behData
% 
% behNames = ['pp','cue',behVarNames]; % not DVs
...
behTab(isnan(many_cond), 2:end) = {NaN};
behNames = behTab.Properties.VariableNames; % update


% Define the categories
cueCategories = categorical(behTab.cue, [-1, 0, 1], {'invalid', 'neutral', 'valid'});

% Add to the table
behTab.cueCategory = cueCategories;

% Define the categories
certCategories = categorical(behTab.certainty, [1, 2, 3], {'maybe', 'probably', 'certain'});

% Add to the table
behTab.certCategory = certCategories;
...
%% saving file
    ...
    fold = 'G:\cue_task\analysis\Data\General';

save (fullfile(fold, [p '_sorted_t']),...
        'many_cond', 'cert',  'cue_and_cert', 'amount_of_each_answers',...
        'combined_labels', 'freq_correct', 'freq_ch', ...
        't_in_cell', 'behData',  'cue_and_e', 'e_or_c', 'isLogistic',...
        'cue_m', 'blocks', 'RT_p', 'pp', 'lr_resp', 'behTab', 'glmeArgs', 'behNames');


save (fullfile(fold, [p 'for_d']),...
        'many_cond', 'cert',  'cue_and_cert', 'amount_of_each_answers',...
        'combined_labels', 'freq_correct', 'freq_ch', ...
        't_in_cell', 'behData',  'cue_and_e', 'e_or_c', 'isLogistic',...
        'cue_m', 'blocks', 'RT_p', 'pp', 'lr_resp', 'data_for_d', 'glmeArgs', 'behNames');